...
namespace Sample
{
	SampleEnvironmentMapping::SampleEnvironmentMapping()
	{
        ...
        reflectionCube = std::make_unique<GameObject>(std::make_shared<Mesh::Cube>(0.8f, 0.8f, 0.8f), reflectionMaterial);
	...
	{
		reflectionCube->GetTransform().GetRotation().y += 10 * deltaTime;
		refractionCube->GetTransform().GetRotation().x += 10 * deltaTime;
	}
